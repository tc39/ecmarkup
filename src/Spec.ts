...
export default class Spec {
  ...
  private async buildMultipage() {
    this.log('Creating multipage build...');

    const doc = this.doc.cloneNode(true) as Document;

    const links = new Map<string, string>();
    const linksForJson: { [key: string]: string[] } = {};
    const registerLinks = (page: string, node: Element) => {
      const all = [];
      if (node.id) {
    ...
    pages.set('index', doc.createElement('div'));
    let hasIntro = false;
    Array.from(doc.querySelector('#spec-container')!.children).forEach(c => {
      if (!hasIntro) {
    ...
    if (!hasIntro) {
      this.warn({
        type: 'global',
        ruleId: 'multipage-intro',
        message: 'Multipage build was enabled but no introduction was found.',
      });
    }

    // hack: add index div to document so below rewrites can reach it.
    // don't perform restructuring after this point.
    doc.body.appendChild(pages.get('index')!);
    ...
    [
      ...
      Array.from(doc.querySelectorAll(type)).forEach(e => {
...
<html>
  ...
  <body>
    ${doc.querySelector('#menu-toggle')!.outerHTML}
    ${doc.querySelector('#menu-spacer')!.outerHTML}
...
`;
    ...
    await Promise.all(
      [...pages].map(([id, e]) => {
        return utils.writeFile(path.join(outDir, 'multipage', `${id}.html`), format(e));
      })
    );
  }
