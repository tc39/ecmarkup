...
function Xref(spec, node) {
  this.spec = spec;
  this.node = node;
}

Xref.prototype.build = function() {
  var xref = this.node;
  var href = xref.getAttribute('href');
  var aoid = xref.getAttribute('aoid');
  ...
  if(href) {
    ...
    var entry = this.spec.biblio.clauses[href.slice(1)] || this.spec.externalBiblio.clauses[href.slice(1)];
    if(entry) {
    ...
    entry = this.spec.biblio.ops[aoid] || this.spec.externalBiblio.ops[aoid];
