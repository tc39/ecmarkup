...
function Clause(spec, node) {
  ...
  var aaid = node.getAttribute('aaid')
  if(aaid) {
    this.spec.biblio.ops[aaid] = '#' + this.id;
  }
}
...
function textNodesUnder(node) {
  ...
  for (node=node.firstChild; node; node=node.nextSibling) {
    ...
    else if(node.nodeName.indexOf("EMU-") !== 0 && node.nodeName !== "PRE" && node.nodeName !== "CODE") all = all.concat(textNodesUnder(node));
  }
