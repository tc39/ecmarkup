...
function Production(spec, node) {
  ...
  if(!this.spec.biblio.productions[this.name]) {
    this.primary = true;
    this.id = 'prod-' + this.name
    this.spec.biblio.productions[this.name] = { id: this.id };
  }
}

Production.prototype.build = function() {
  var ntNode = this.spec.doc.createElement('emu-nt');
  ntNode.innerHTML = '<a href="#' + this.id + '">' + this.name + '</a>';
