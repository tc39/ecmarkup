...
const addIdReferences = require('./idReferences');
...
module.exports = class Spec {
  ...
  build() {
    let p = this.buildAll('emu-import', Import)
      ...
      .then(this.addIdReferences.bind(this));
  ...
  addIdReferences() {
    this._log('Adding references tables for all elements with IDs...');

    addIdReferences(this.spec.doc);
  }
