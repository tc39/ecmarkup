...
Spec.prototype.build = function(opts) {
  opts = opts || {};

  if(!opts.hasOwnProperty('toc')) {
    opts.toc = true;
  }
  ...
  for(var i = 0; i < xrefs.length; i++) {
    ...
    if(xref.textContent.trim() === '') {
      if(xref.hasAttribute('title')) {
        xref.innerHTML = '<a href="#' + entry.id + '">' + entry.title + '</a>';
      } else {
        xref.innerHTML = '<a href="#' + entry.id + '">' + entry.number + '</a>';
      }
    } else {
      xref.innerHTML = '<a href="#' + entry.id + '">' + xref.innerHTML + '</a>';
    }
  }
}
